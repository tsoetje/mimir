...
func TestPoolInstrumenter(t *testing.T) {
	for _, impl := range []struct {
		name string
		new  func(t *Tracker, reserve func(*Reserver), item func(*Allocator) *obj) any
		get  func(pool any) *obj
		put  func(pool any, o *obj)
	}{{
		name: "SyncPool",
		new: func(t *Tracker, reserve func(*Reserver), item func(*Allocator) *obj) any {
			p := NewInstrumentedSyncPool(t, reserve, func(a *Allocator) any {
				return item(a)
			})
			return &p
		},
		...
		name: "ZeroPool",
		new: func(t *Tracker, reserve func(*Reserver), item func(*Allocator) *obj) any {
			p := NewInstrumentedZeroPool(t, reserve, item)
			return &p
		},
		get: func(pool any) *obj {
		...
		t.Run(impl.name, func(t *testing.T) {
			...
			p := impl.new(tracker, reserveObj, newObj)
			...
			// Panics when allocating past the reserved capacity
			tracker = InstrumentConfig{Percentage: 100}.Tracker(prometheus.NewRegistry())
			p = impl.new(tracker, func(r *Reserver) {
				reserveObj(r)
				r.bytesCap /= 2
			}, newObj)
			require.Panics(t, func() { impl.get(p) })
		})
	}
}
